...
export default function ChatPaneHtml({
  companyId,
  chatId,
  workspaceId,
  workspaceName,
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/generate-email-html", {
        method: "POST",
        body: formData,
      });
      ...
      if (data.output) {
        ...
        if (output.draft) {
          setDraft(output.draft);
        } else {
          console.error("[ChatPaneHtml] No draft found in output");
        }
